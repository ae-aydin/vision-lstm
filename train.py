...
def train_one_epoch(
    model, loader, criterion, optimizer, scheduler, device, epoch, total_epochs, amp_ctx
):
    model.train()
...
def main():
    ...
    if args.compile:
        print(
            "Compiling model with torch.compile — first epoch will be slow (~5-10 min for tracing)."
        )
    ...
    print(
        f"Traversal  : {args.traversal}  |  conv: causal1d  |  device: {device}  |  amp: {amp_label}  |  compile: {args.compile}"
    )
